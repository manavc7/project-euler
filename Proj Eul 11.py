...
for i in range(len(list_num)):
    # Check right
    if i % 20 <= 16:
        max_product = max(max_product, product_of_four(list_num[i:i + 4]))

    # Check down
    if i // 20 <= 16:
        max_product = max(max_product, product_of_four(list_num[i:i + 4 * 20:20]))

    # Check diagonally (right-down)
    if i % 20 <= 16 and i // 20 <= 16:
        max_product = max(max_product, product_of_four(list_num[i:i + 4 * 20 + 4*1:20 + 1]))
